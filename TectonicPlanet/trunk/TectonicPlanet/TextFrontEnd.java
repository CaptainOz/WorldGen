...
public class TextFrontEnd implements Runnable {
  ...
  public static void main(String[] arg) {
    try {
      TextFrontEnd tfe=new TextFrontEnd();
		  if (arg.length==1) tfe.init(arg[0]);
		  else if (arg.length==2) tfe.init(arg[0], arg[1]);
			else System.out.println("Usage: java TextFrontEnd <timesteps> [filename]");
    } catch(Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void newWorld() {
		...
		for (int i=0; i<world.getNumPoints(); i++)
		  world.getPoint(i).setColor(world.getColorMap().map(world.getPoint(i).getSurfaceHeight()));
	}
    ...
    for (int i=0; i<world.getNumPoints(); i++)
      world.getPoint(i).setColor(world.getColorMap().map(world.getPoint(i).getSurfaceHeight()));
		...
		// Always save the jpgs
    if (world.getEpoch()%20==0) world.saveJPGsequence();	
		// Always save the planetfiles
