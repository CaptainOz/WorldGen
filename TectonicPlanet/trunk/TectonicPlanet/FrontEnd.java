...
public class FrontEnd extends JPanel implements MouseListener, MouseMotionListener, ActionListener, ChangeListener, Runnable {
    ...
    public void actionPerformed( ActionEvent e ){
        ...
        if( source.equals( flowMenuItem )
                ...
                || source.equals( stressColorMenuItem ) )
            repaint();
    }
        ...
        if( world != null ){
            ...
            for( int i = 0; i < world.getNumPoints(); i++ ){
                ...
                if( exaggerateMenuItem.isSelected() )
                    world.getPoint( i ).rotPos().scale( (world.getPlanetRadius() - 200 + world.getPoint( i ).getSurfaceHeight() * 100) / world.getPoint( i ).getHeight() );
            }
                ...
                for( int i = 0; i < tempVec.size(); i++ ){
                    ...
                    if( lp.getA().getPlate() != lp.getB().getPlate() ){
                        ...
                        if( plateGridMenuItem.getState() ){
                            if( plateColorMenuItem.isSelected() )
                                g.setColor( lp.getA().getPlate().getCol() );
                            ...
                            if( lp.getA().mean != lp.getB().mean )
                                g.setColor( Color.red );
                            if( heightColorMenuItem.isSelected() )
                                g.setColor( lp.col );
                            if( stressColorMenuItem.isSelected() ){
                                ...
                                if( c < -255 )
                                    c = -255;
                                if( c >= 0 )
                                    g.setColor( new Color( 255, 255 - c, 255 - c ) );
                                else
                                    g.setColor( new Color( 255 + c, 255 + c, 255 ) );
                                if( lp.broken )
    ...
    private void timeStep(){
        ...
        info.setText( world.getLinkSystem().size() + " links, " + world.getNumTets() + " tets, " + world.getNumPoints() + " points, " + world.getNumPlates() + " plates" );
        if( pointViewer != null )
            pointViewer.updateDetails();

        if( world.getEpoch() % 20 == 0 && saveImageSequenceMenuItem.getState() )
            world.saveJPGsequence();
        if( world.getEpoch() % 20 == 0 )
            world.save();
