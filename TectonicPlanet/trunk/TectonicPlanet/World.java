...
public class World {
  // Physics bits
    ...
    private TecPoint centerOfPlanet = null;
    private Vector3d omega = null;  // Speed of planet rotation
    
  // Delaunay bits
    private int gridSize;
    private HashSet[][][] gridBox;
    private LinkSystem linkSystem;
    
  // Mantle flow arrows
    private double[] mStrength;
    private Point3d[] mPoint;
    private int mantlePoints;
  ...
  // Map saving bits
    ...
    private int tetGridSize;
    private HashSet[][][] tetGridBox;
  ...
  private void initColors() {
    ...
    colorMap.add(10.00000f, new Color(1.000000f, 1.000000f, 0.870588f));
  	colorMap.add(15.4f, Color.red);*/
    colorMap.add(-5.6f, Color.black);
  	//colorMap.add(-1f, new Color(20,20,120));
  	colorMap.add(-0.01f, Color.blue);
    colorMap.add(0, new Color(50,70,25));
    colorMap.add(1.5f, new Color(75,155,74));
    colorMap.add(2.5f, new Color(125,190,80));
    colorMap.add(4, new Color(175,140,100));
    colorMap.add(7, new Color(213,185,135));
    colorMap.add(10, new Color(255,255,255));
  }
    
  private void initPoints(int numPlates) {
    // Build a new planet
  	Tet.planetRadius = planetRadius;
    
    // First place numPlates random plates
  	for (int i_1_ = 0; i_1_ < numPlates; i_1_++) {
  	    Vector3d vector3d	= getRandomVector();
  	    vector3d.scale(planetRadius);
  	    addPlate(new TecPlate(vector3d.x, vector3d.y, vector3d.z));
  	    getPlate(i_1_).densityTweak = Math.random() * 0.15;
  	}
  	km = 360;	// Point spacing, in km  (120km=75miles, 160km=100miles)
  
  	double randomness = 1.0; // The random jiggle given to the position of each point
  	double equatorCircum = 2*Math.PI * planetRadius;
  	System.out.println("Planet circumference="+equatorCircum+" km");
  	int latStep = (int) (equatorCircum / (double) (km * 2));
  	System.out.println("Grid spacing of "+km+" km gives "+latStep+" points from pole to pole\n");
  	...
  	for (int latCount = 0; latCount <= latStep; latCount++) {
  	  ...
  	  double circum = (2*Math.PI * Math.cos(Math.toRadians(lat)) * planetRadius);
  	  int numPoints = (int) (circum / (double) km); // The number of points which will fit on thisline of latitude
  	  for (int i_8_ = 0; i_8_ <= numPoints; i_8_++) {
    		double lon = (double) i_8_ / (double) (numPoints + 1) * 360.0;
    		TecPoint tecpoint = new TecPoint(lat + Math.random() * randomness, lon + Math.random() * randomness, planetRadius, false, epoch);
        // Find closest plate
    		int cp = 0;
    		double cdist = tecpoint.getPos().distance(getPlate(0).getPos());
    		for (int i_12_ = 1; i_12_ < plates.size(); i_12_++) {
    ...
    // Make some of the plates continental
  	for (int i_13_ = 0; i_13_ < numPlates; i_13_++) {
  	  boolean ocean = false;
  	  if (Math.random() < 0.75)	ocean = true;
  	  for (int i_14_ = 0; i_14_ < getPlate(i_13_).getPoints().size(); i_14_++) {
    		if (ocean) getPlate(i_13_).getPoint(i_14_).makeNewOceanFloor();
    		else getPlate(i_13_).getPoint(i_14_).addLayer((float)(20.8*(0.9+0.1*Math.random())), 2.6f);
  	  }
  	}
    
		// The rotation vector of the planet (for Coriolis Effect)
  	omega = new Vector3d(0.0, 0.072722, 1.0E-13); // In radians/sec, should be 0.000072722
  	System.out.println("\n");
    ...
    // Initialise the gridBox system
  	gridSize = (int) Math.ceil(-1.0 + planetRadius / ((double) km * 1.1));		// possibly change this to      -1+planetRadius/(km*1.1)
  	System.out.println("Grid is "+gridSize+"^3");
  	gridBox = new HashSet[gridSize][gridSize][gridSize];
  	for (int i_17_ = 0; i_17_ < gridSize; i_17_++) {
  	  for (int i_18_ = 0; i_18_ < gridSize; i_18_++) {
  		  for (int i_19_ = 0; i_19_ < gridSize; i_19_++)
  		    gridBox[i_17_][i_18_][i_19_] = null;
  	  }
  	}
    ...
    for (int i_20_ = 0; i_20_ < tets.size(); i_20_++)
	    gridBoxAdd((Tet) tets.get(i_20_));
  ...
  public void randomiseUpwellings() {
    for (int i_21_ = 0; i_21_ < mantlePoints; i_21_++) {
  	  boolean ok = true;
      // Shuffle the upwelling point randomly, until it _isn't_ too close to the other points
  	  do {
    		...
    		Vector3d vector3d = getRandomVector();
    		vector3d.scale(planetRadius);
    		mPoint[i_21_] = new Point3d(vector3d.x, vector3d.y, vector3d.z);
        // Check for other points close by
    		for (int i_22_ = 0; i_22_ < i_21_; i_22_++) {
    		  if (i_22_ != i_21_ && mPoint[i_22_].distance(mPoint[i_21_]) < 3000.0)
            ok = false;
    		}
  	  } while (!ok);
  	}
  ...
  public void supercontinentBreakup() {
    ...
    for (int i_21_ = 0; i_21_ < mantlePoints; i_21_++) {
      System.out.println("Moving manthle point #"+(i_21_+1)+" of "+mantlePoints);
  	  boolean ok = true;
      // Shuffle the upwelling point randomly, until it _isn't_ too close to the other points
  	  do {
    		ok = true;
        ...
        for (int p=0; p<points.size(); p++) {
          ...
          double dist=mPoint[i_21_].distance(tp.getPos());
          if (dist<3000)
        ...
        if (Math.random()<Math.pow(8/vol,2) && Math.random()<0.999) ok=false;  // Make sure we don't get stuck forever on a young world
  	  } while (!ok);
  	}
    System.out.println("...done!");
  }
  ...
  public void initTetGridBoxSystem() {
    // Initialise the tetGridBox system
  	tetGridSize = 15;
  	System.out.println("tetGrid is "+tetGridSize+"^3");
  	tetGridBox = new HashSet[tetGridSize][tetGridSize][tetGridSize];
  	for (int i = 0; i < tetGridSize; i++) {
  	  for (int j = 0; j < tetGridSize; j++) {
        for (int k = 0; k < tetGridSize; k++)
  		    tetGridBox[i][j][k] = null;
  	  }
    }
  }
  ...
  final public void load(String filename) {
    ...
    try {
	    ...
	    mStrength = new double[mantlePoints];
	    mPoint = new Point3d[mantlePoints];
	    for (int i = 0; i < mantlePoints; i++) {
  			mPoint[i] = new Point3d();
  			mStrength[i] = datainputstream.readDouble();
  			mPoint[i].x = datainputstream.readDouble();
  			mPoint[i].y = datainputstream.readDouble();
  			mPoint[i].z = datainputstream.readDouble();
	    }
      ...
      initColors(); // Reset the colours to the built-in ones
	    ...
	    gridSize = (int) Math.ceil(-1.0 + planetRadius / ((double) km * 1.1));
	    System.out.println("Grid is "+gridSize+"^3");
	    gridBox = new HashSet[gridSize][gridSize][gridSize];
	    for (int i_36_ = 0; i_36_ < gridSize; i_36_++) {
			for (int i_37_ = 0; i_37_ < gridSize; i_37_++) {
				for (int i_38_ = 0; i_38_ < gridSize; i_38_++)
					gridBox[i_36_][i_37_][i_38_] = null;
				}
			}
	    initTetGridBoxSystem();
  ...
  public void save() {
  	...
  	if (saveFile == null) {
	    ...
	    try {
        if(!new File("TectonicPlanet/Worlds").exists()) new File("TectonicPlanet/Worlds").mkdirs();
	      int num=1;
	      while (new File("TectonicPlanet/Worlds/World"+num+".world").exists()) {num++;}
	...
	try {
	    File file = new File(saveFile.getCanonicalPath()+".bak");
	    if (saveFile.exists()) {
			  // Remove the backup, if it exists
    		if (file.exists()) file.delete();
    		saveFile.renameTo(file);
	    }
	    DataOutputStream dataoutputstream = new DataOutputStream(new FileOutputStream(saveFile));
			// Epoch
	    dataoutputstream.writeInt(epoch);
			// Planet radius
	    dataoutputstream.writeDouble(planetRadius);
			// Km
	    dataoutputstream.writeInt(km);
			// Mantle points
	    dataoutputstream.writeInt(mantlePoints);
	    for (int i = 0; i < mantlePoints; i++) {
    		dataoutputstream.writeDouble(mStrength[i]);
    		dataoutputstream.writeDouble(mPoint[i].x);
    		dataoutputstream.writeDouble(mPoint[i].y);
    		dataoutputstream.writeDouble(mPoint[i].z);
	    }
			// ColorMap
	    dataoutputstream.writeInt(colorMap.size());
	    for (int i = 0; i < colorMap.size(); i++) {
    		dataoutputstream.writeDouble(colorMap.getValue(i));
    		dataoutputstream.writeInt(colorMap.getColor(i).getRGB());
	    }
			// Number of plates
	    dataoutputstream.writeInt(plates.size());
			// Number of points
	    ...
	    for (int i = 0; i < points.size(); i++) {
    		...
    		// And the number of its plate
    		int plateNum = -1;
    		for (int i_41_ = 0; i_41_ < plates.size(); i_41_++) {
  ...
  public boolean hasName() {return saveFile != null; }
    
  private void initGridBox() {
    // Init the gridBox system
    gridSize=(int)Math.ceil(-1+planetRadius/(km*1.1));
    System.out.println("Grid is "+gridSize+"^3");
    gridBox=new HashSet[gridSize][gridSize][gridSize];
  ...
  public void delaunay() {
    long time = System.currentTimeMillis();
  	...
  	for (int i = 0; i < gridSize; i++)
  	  for (int j = 0; j < gridSize; j++)
    		for (int k = 0; k < gridSize; k++)
    		  if (gridBox[i][j][k] != null)
      			gridBox[i][j][k].clear();
	
    // Put all the points in the gridBoxes
  	for (int i = 0; i < points.size(); i++)
	    gridBoxAdd((TecPoint) points.get(i));
    ...
    if (tets != null) {
      ...
      // Make random tets, then check if they're ok
	    do {
    		goodTet = true;
    		startTet = (Tet) tets.get((int) (Math.random()* (double) tets.size()));
    		// Check if tet is ok
    		for (int i = 0; i < points.size() && goodTet; i++) {
	    ...
	    linkSystem.addLink(startTet.b, startTet.d);
	    linkSystem.addLink(startTet.c, startTet.d);
	    ...
	    Tet tet1 = new Tet(centerOfPlanet, getPoint(0), getPoint(1), getPoint(2));
	    tet1.calc();
			// Check if it's ok
	    ...
	    if (ok) {
    		...
    		tets.add(tet1);
    		linkSystem.addLink(tet1.b, tet1.c);
    		linkSystem.addLink(tet1.b, tet1.d);
    		linkSystem.addLink(tet1.c, tet1.d);
    		activeTets.add(tet1);
          ...
          do {p2=(int)(Math.random()*nearby.size());} while (p2==p1);
          do {p3=(int)(Math.random()*nearby.size());} while (p3==p1 && p3==p2);
          tet1=new Tet(centerOfPlanet,(TecPoint)nearby.get(p1),
                                      (TecPoint)nearby.get(p2),
                                      (TecPoint)nearby.get(p3));
          tet1.calc();
  		    for (int i_56_ = 0; i_56_ < points.size(); i_56_++) {
      			if (ok
      			    && !getPoint(i_56_)
      				    .equals((TecPoint) nearby.get(p1))
      			    && !getPoint(i_56_)
      				    .equals((TecPoint) nearby.get(p2))
      			    && !getPoint(i_56_)
      				    .equals((TecPoint) nearby.get(p3))
    ...
    while (activeTets.size() > 0 && tetNum<activeTets.size()) {
	    ...
	    if (Math.random() < 0.01) System.out.print(".");
	    t1 = null;
	    t2 = null;
	    t3 = null;
      ...
      // Can we use link tet.b<->tet.c?
	    if (linkSystem.getCount(tet.b, tet.c) <= 1) {
        ...
        while (iter.hasNext() && t1==null) {
    		//for (int i = 0; t1 == null && i < nearby.size(); i++) {
    		  ...
    		  if (!tecpoint.equals(tet.a) && !tecpoint.equals(tet.b) && !tecpoint.equals(tet.c) && !tecpoint.equals(tet.d)) {
      			Tet tet_63_ = new Tet(centerOfPlanet, tet.b, tet.c, tecpoint);
      			tet_63_.calc();
            ...
            while (ok && iter2.hasNext()) {
              ...
              tets.add(t1);
              activeTets.add(t1);
              linkSystem.addLink(t1.b, t1.c);
              linkSystem.addLink(t1.b, t1.d);
              linkSystem.addLink(t1.c, t1.d);
      			}
    		  }
    		}
	    }
      // Can we use link tet.c<->tet.d?
	    if (linkSystem.getCount(tet.c, tet.d) <= 1) {
        ...
        while (iter.hasNext() && t2==null) {
    		//for (int i = 0; t2 == null && i < nearby.size(); i++) {
    		  TecPoint tecpoint = (TecPoint) iter.next();//nearby.get(i);
    		  if (!tecpoint.equals(tet.a) && !tecpoint.equals(tet.b) && !tecpoint.equals(tet.c) && !tecpoint.equals(tet.d)) {
            Tet tet_67_ = new Tet(centerOfPlanet, tet.c, tet.d, tecpoint);
      			tet_67_.calc();
            // Now check against all nearby points
      			boolean ok = true;
            iter2=nearby.iterator();
            while (ok && iter2.hasNext()) {
                ...
                ok = false;
      			...
      			if (ok) {
    			    ...
    			    tets.add(t2);
    			    activeTets.add(t2);
    			    linkSystem.addLink(t2.b, t2.c);
    			    linkSystem.addLink(t2.b, t2.d);
    			    linkSystem.addLink(t2.c, t2.d);
            }
    		  }
    		}
	    }
      // Can we use link tet.b<->tet.d?
	    if (linkSystem.getCount(tet.b, tet.d) <= 1) {
        ...
        //for (int i = 0; t3 == null && i < vector_70_.size(); i++) {
  		    ...
  		    if (!tecpoint.equals(tet.a) && !tecpoint.equals(tet.b) && !tecpoint.equals(tet.c) && !tecpoint.equals(tet.d)) {
            Tet tet_71_ = new Tet(centerOfPlanet, tet.b, tet.d, tecpoint);
            tet_71_.calc();
            // Now check against all nearby points
      			boolean ok = true;
            iter2=nearby.iterator();
            while (ok && iter2.hasNext()) {
                ...
                ok = false;
      			...
      			if (ok) {
    			    ...
    			    tets.add(t3);
    			    activeTets.add(t3);
    			    linkSystem.addLink(t3.b, t3.c);
    			    linkSystem.addLink(t3.b, t3.d);
    			    linkSystem.addLink(t3.c, t3.d);
            }
    		  }
        }
      }
	    if (linkSystem.getCount(tet.b, tet.c) >= 2 && linkSystem.getCount(tet.b, tet.d) >= 2 && linkSystem.getCount(tet.c, tet.d) >= 2)
  ...
  public void addPlate(TecPlate tecplate) {plates.add(tecplate);  }
  public int getPlateNum(TecPlate tp) {
    for (int i=0; i<plates.size(); i++)
      if (tp.equals(plates.get(i))) return i;
    return -1;
  }
  public TecPlate getPlate(int i) {return (TecPlate) plates.get(i);}
  
  public int getNumPlates() {	return plates.size();    }
  ...
  public Vector3d getMantleFlow(double x, double y, double z) {
    // Find the closest and second-closest upwelling points
  	...
  	int scp = -1;
  	double scd;
  	double cd = scd = Math.pow(planetRadius * 2.0 + 100.0, 2.0);
  	for (int i_80_ = 0; i_80_ < mantlePoints; i_80_++) {
	    double dist = mPoint[i_80_].distanceSquared(point3d);
	    if (dist <= cd) {
        // Move cd down to scd
    		scd = cd;
    		scp = cp;
  	...
  	double speed;
  	if (ratio <= 0.1)    speed = ratio * 10.0;
    else    speed = (0.4 - (ratio - 0.1)) / 0.4;
    Vector3d out = new Vector3d(x - mPoint[cp].x, y - mPoint[cp].y, z - mPoint[cp].z);
    out.normalize();
    out.scale(speed*100);
    return out;
  }
  ...
  private int getGridBoxX(double d) {
    return (int) Math.round((d + planetRadius) * (double) (gridSize - 1) / (planetRadius * 2.0));
  }
  ...
  public void gridBoxAdd(TecPoint tecpoint) {
  	...
  	try {
	    if (gridBox[x][y][z] == null)
        gridBox[x][y][z] = new HashSet(100);
  ...
  private HashSet getGridBox(Point3d p) {
    ...
    return gridBox[x][y][z];
  }
	private void gridBoxRemove(TecPoint p) {
    int x=getGridBoxX(p);
    int y=getGridBoxY(p);
    int z=getGridBoxZ(p);
    if (gridBox[x][y][z]!=null) gridBox[x][y][z].remove(p);
	}
  ...
  private HashSet pointsSurrounding(int x, int y, int z) {
    ...
    for (int i=-1; i<=1; i++)
      for (int j=-1; j<=1; j++)
        for (int k=-1; k<=1; k++)
          if (x+i>=0 && x+i<gridSize && y+j>=0 && y+j<gridSize && z+k>=0 && z+k<gridSize && gridBox[x+i][y+j][z+k]!=null)
            out.addAll(gridBox[x+i][y+j][z+k]);
    return out;
  }
    ...
    for (int i=Math.min(x1,x2)-1; i<=Math.max(x1,x2)+1; i++)
      for (int j=Math.min(y1,y2)-1; j<=Math.max(y1,y2)+1; j++)
        for (int k=Math.min(z1,z2)-1; k<=Math.max(z1,z2)+1; k++)
          if (i>=0 && i<gridSize && j>=0 && j<gridSize && k>=0 && k<gridSize && gridBox[i][j][k]!=null)
            out.addAll(gridBox[i][j][k]);
    return out;
  }
  ...
  private int getGridBoxSize(int i, int i_117_, int i_118_) {
    if (gridBox[i][i_117_][i_118_] == null)    return 0;
    return gridBox[i][i_117_][i_118_].size();
  }
  //private TecPoint getGridBoxPoint(int x, int y, int z, int p) {
  //  return (TecPoint)gridBox[x][y][z].get(p);
  //}
	public LinkSystem getLinkSystem() {return linkSystem;}
	public int getNumMantlePoints() {return mantlePoints;}
	public Point3d getMantlePoint(int i) {return mPoint[i];}
	public int getNumTets() {return tets.size();}
	public int getEpoch() {return epoch;}
	
  private void calcMantleForceForTecPoint(TecPoint p) {
    p.mantleFlow = getMantleFlow(p.getPos());
    if (p.mantleForce==null) p.mantleForce=new Vector3d();
  ...
  public void timeStep() {
    ...
    if (foundBadness) calculateEdgeLinkPairs();
    ...
    // Invalidate every edge point
  	Iterator iterator = linkSystem.getIterator();
      ...
      firstPoint=getPoint(p);
        ...
        firstPoint.lms=landmassSections.size()-1;
				...
				for (int i=0; i<lms.size(); i++) {
				  HashSet pointsToAdd=linkSystem.getLinkedPoints((TecPoint)lms.get(i),pointsSurrounding(((TecPoint)lms.get(i)).getPos()));
            ...
            // Check for obduction
              ...
              HashSet mPoints=new HashSet(tecplate.getPoints().size()+tecplate2.getPoints().size());
    			    for (int i = 0; i < tecplate.getPoints().size(); i++) {
        				...
        				if (dist < squashSize) mPoints.add(tecpoint_159_);
              }
              ...
              ArrayList squishTets=new ArrayList();
                ...
                if (tet.getPlate()==tecplate || tet.getPlate()==tecplate2)
                  if (mPoints.contains(tet.b) || mPoints.contains(tet.c) || mPoints.contains(tet.d)) squishTets.add(tet);
              }
              // Place the 'area' of each tet into its 'oldArea'
                ...
                tet.oldArea=tet.area;
                  ...
                  tecpoint_159_.setHeight(planetRadius);
                  ...
                  tecpoint_162_.setHeight(planetRadius);
    ...
    HashSet edgePoints=new HashSet(points.size());
  	Iterator it = linkSystem.getIterator();
  	while (it.hasNext()) {
    ...
    for (int j=0; j<links.size(); j++) {
      ...
      if (lp.getA().getPlate()==lp.getB().getPlate() && getPlateNum(lp.getA().getPlate())!=-1) {
        ...
        lp.linkWidth=linkSystem.linkWidth(lp);
      }
    }
...
//System.out.println("Broke a link");
                ...
                while (brokenPoints.size()>0) {
                  ...
                  if (brokenPoints.size()==oldBrokenPointsSize) {
                    ...
                    while (brokenPoints.size()>0) {
                      ...
                      for (int k=0; k<linkedPoints.size(); k++) {
                        ...
                        else if (unmovedPoints.contains(linkedPoints.get(k))) countUnmoved++;
                        else System.out.println("!!!!!One of the brokenPoints wasn't in Moved _or_ Unmoved!!!!!");
                      }
                  ...
                  for (int j=0; j<plateLinks.size(); j++) {
                    ...
                    if (lp.getA().broken && !lp.getB().broken && lp.getB().getPlate()==newPlate && !remember.contains(lp.getA())) remember.add(lp.getA());
                      else if (lp.getB().broken && !lp.getA().broken && lp.getA().getPlate()==newPlate && !remember.contains(lp.getB())) remember.add(lp.getB());
                  }
                  ...
                  for (int j=0; j<plateLinks.size(); j++) {
                    lp=(LinkPair)plateLinks.get(j);
                    if (lp.getA().broken && !lp.getB().broken && lp.getB().getPlate()==plate && !remember.contains(lp.getA())) remember.add(lp.getA());
                      else if (lp.getB().broken && !lp.getA().broken && lp.getA().getPlate()==plate && !remember.contains(lp.getB())) remember.add(lp.getB());
                  }
    ...
    if (plates.size()>2) {
      ...
      Iterator iter=linkSystem.getIterator();
      while (iter.hasNext()) {
      ...
      if (capturedPlates.size()>0) System.out.println("Found "+capturedPlates.size()+" captured plates");
    ...
    for (int i=0; i<accPlates.size(); i++) {
      ...
      while (iter.hasNext()) {
        ...
        if (otherPlate!=null) {
          if (linkWidths.get(otherPlate)==null) {linkWidths.put(otherPlate,new Double(0));}
          double w=((Double)linkWidths.get(otherPlate)).doubleValue();
          linkWidths.put(otherPlate,new Double(w+linkSystem.linkWidth(lp)));
        }
      }
    ...
    while (it.hasNext()) {
      ...
      if (tecpoint.heightAboveSeaLevel()<0) gradLimit=gradientLimitInSea;
      ...
      if (tecpoint2.heightAboveSeaLevel()<0) gradLimit=gradientLimitInSea;
      ...
      if (p.volCap<0) {
        ...
        while (liftedVol>1 && count<1000) {
          ...
          for (int j=0; j<linkedPoints.size(); j++) {
            ...
            if (tempPoint.getSurfaceHeight()<lh) { lowest=tempPoint; lh=lowest.getSurfaceHeight(); }  // Found a new lowest point
          }
          ...
          p=lowest;
          linkedPoints=linkSystem.getPointLinks(p);
          count++;
        }
        if(count>=1000) {
          ...
          for (int j=0; j<points.size(); j++) {
            getPoint(j).add(liftedVol/planetSurfaceArea,liftedDens);
          }
        }
      }
    }
    ...
    for (int rep=0; rep<1; rep++) {
        ...
        double dist= linkpair.getLength();
        double gradient=diff/dist;
        if (Math.random()<0.0001) System.out.println("             Gradient="+gradient);
        if (tecpoint2.heightAboveSeaLevel()>1400) {  // Where "mountains" start - high erosion above here
          if (Math.random()<1.5) {
            ...
            double depthToAdd=depthToRemove*tecpoint2.getArea()/tecpoint.getArea();
            if (depthToRemove<0 || depthToAdd<0) {System.out.println("depthToRemove="+depthToRemove+",  depthToAdd="+depthToAdd+", highArea="+tecpoint2.getArea()+", lowArea="+tecpoint.getArea()); System.exit(1);}
            tecpoint2.remove(depthToRemove);
    ...
    if (meltcount>0) System.out.println("Melted the base off "+meltcount+" points due to mountains being too high");
  ...
  public void redelaunay() {
    ...
    for (int i = 0; i < tets.size(); i++) {
	    Tet tet = (Tet) tets.get(i);
	    //if (!tet.b.isValid() || !tet.c.isValid() || !tet.d.isValid()) {
        if (!checkIfValid(tet)) invalidTets.add(tet);
      //}
    }
    ...
    if (invalidTets.size() % 2 == 1) {
      ...
      // Bad tet! Remove it, and tidy the linksystem
	    tets.remove(tet);
	    linkSystem.removeLink(tet.b, tet.c);
	    linkSystem.removeLink(tet.b, tet.d);
	    linkSystem.removeLink(tet.c, tet.d);
    }
    ...
    // Build list of active tets
  	...
  	for (int i = 0; i < tets.size(); i++) {
       ...
       || linkSystem.getCount(tet.b, tet.d) < 2
       || linkSystem.getCount(tet.c, tet.d) < 2) activeTets.add(tet);
    }
    ...
    int tetNum=0;
  	while (activeTets.size() > 0 && !failedToExpand && tetNum<activeTets.size()) {
	    if (!failedToExpand) tet = (Tet)activeTets.get(tetNum);    //((Tet)activeTets.get((int) (Math.random() * (double) activeTets.size())));
	    if (Math.random() < 0.01) System.out.print(".");
      ...
      // Can we use link tet.b<->tet.c?
        ...
        //for (int i = 0; t1 == null && i < nearby.size(); i++) {
           ...
           && linkSystem.getCount(tet.b, tecpoint) <= 1
           && linkSystem.getCount(tet.c, tecpoint) <= 1) {
    				Tet tet_209_ = new Tet(centerOfPlanet, tet.b, tet.c, tecpoint);
    				tet_209_.calc();
            // Now check against all nearby points
    				boolean ok = true;
            iter2=nearby.iterator();
...
//System.out.println("Added new tet");
      ...
      // Can we use link tet.c<->tet.d?
        ...
        //for (int i = 0; t2 == null && i < nearby.size(); i++) {
           ...
           && !tecpoint.equals(tet.c) && !tecpoint.equals(tet.d)
           && linkSystem.getCount(tet.c, tecpoint) <= 1
           && linkSystem.getCount(tet.d, tecpoint) <= 1) {
  					Tet tempTet = new Tet(centerOfPlanet, tet.c, tet.d, tecpoint);
  					tempTet.calc();
            // Now check against all nearby points
  					boolean ok = true;
            iter2=nearby.iterator();
      ...
      // Can we use link tet.b<->tet.d?
        ...
        //for (int i = 0; t3 == null && i < nearby.size(); i++) {
  		    ...
  		    if (!tecpoint.equals(tet.a) && !tecpoint.equals(tet.b)
  			    ...
  			    && linkSystem.getCount(tet.b, tecpoint) <= 1
  			    && linkSystem.getCount(tet.d, tecpoint) <= 1) {
            Tet tempTet = new Tet(centerOfPlanet, tet.b, tet.d, tecpoint);
            tempTet.calc();
            // Now check against all nearby points
    				boolean ok = true;
            iter2=nearby.iterator();
...
//System.out.println("Added new tet");
	    ...
	    if (linkSystem.getCount(tet.b, tet.c) >= 2 &&
		    linkSystem.getCount(tet.b, tet.d) >= 2 &&
		    linkSystem.getCount(tet.c, tet.d) >= 2) {
		      tetNum++;//activeTets.remove(remTetNum);
	    } else {
        System.out.println("Completely failed to expand from tet.\nLinkcounts are "+linkSystem.getCount(tet.b,tet.c)+","+linkSystem.getCount(tet.c,tet.d)+","+linkSystem.getCount(tet.b,tet.d));
				// Detailed error report/debugging info
				if (tets.contains(tet)) {
				  System.out.println("Tet is #"+tets.indexOf(tet)+" of "+tets.size()+" tets");
				} else System.out.println("Tet isn't in tets");
				if (activeTets.contains(tet)) {
				  System.out.println("and #"+activeTets.indexOf(tet)+" of "+activeTets.size()+" activeTets");
				} else System.out.println("Tet isn't in activeTets");
				System.out.print("tet.a=("+tet.a.getX()+","+tet.a.getY()+","+tet.a.getZ()+")");
				if (points.contains(tet.a)) System.out.println(" and is point #"+points.indexOf(tet.a)); else System.out.println(" and isn't in points!");
				System.out.print("tet.b=("+tet.b.getX()+","+tet.b.getY()+","+tet.b.getZ()+")");
				if (points.contains(tet.b)) System.out.println(" and is point #"+points.indexOf(tet.b)); else System.out.println(" and isn't in points!");
				System.out.print("tet.c=("+tet.c.getX()+","+tet.c.getY()+","+tet.c.getZ()+")");
				if (points.contains(tet.c)) System.out.println(" and is point #"+points.indexOf(tet.c)); else System.out.println(" and isn't in points!");
				System.out.print("tet.d=("+tet.d.getX()+","+tet.d.getY()+","+tet.d.getZ()+")");
				if (points.contains(tet.d)) System.out.println(" and is point #"+points.indexOf(tet.d)); else System.out.println(" and isn't in points!");
        if (t1 == null) {
            ...
            while (t1==null && iter.hasNext()) {
              ...
              TecPoint pk = (TecPoint) iter.next();//nearby.get(i);
  				    if (!pk.equals(tet.a)
    					 ...
    					 && !pk.equals(tet.c)
    					 && !pk.equals(tet.d)
    					 && (linkSystem.getCount(tet.b, pk) <= 1)
    					 && (linkSystem.getCount(tet.c, pk) <= 1)) {
                Tet tempTet = new Tet(centerOfPlanet, tet.b, tet.c, pk);
                tempTet.calc();
    		        // Now check against all nearby points
                boolean ok = true;
                int check = 0;
                ...
                while (ok && iter2.hasNext()) {
    				...
    				Tet temp = new Tet(centerOfPlanet, tet.b, tet.c, bestPoint);
    				temp.calc();
        ...
        } else System.out.println("t1 is NOT null");
        if (t2 == null) {
            ...
            while (t1==null && iter.hasNext()) {
              ...
              TecPoint pk = (TecPoint) iter.next();//nearby.get(i);
  				    if (!pk.equals(tet.a)
    					 ...
    					 && !pk.equals(tet.d)
    					 && (linkSystem.getCount(tet.c, pk) <= 1)
    					 && (linkSystem.getCount(tet.d, pk) <= 1)) {
    						Tet tempTet = new Tet(centerOfPlanet, tet.c, tet.d, pk);
    						tempTet.calc();
    		        // Now check against all nearby points
    						boolean ok = true;
    						int check = 0;
        ...
        if (t3 == null) {
          System.out.println("t3 is null");
            ...
            while (t1==null && iter.hasNext()) {
              ...
              TecPoint pk = (TecPoint) iter.next();//nearby.get(i);
  				    if (!pk.equals(tet.a) && !pk.equals(tet.b)
    					 ...
    					 && (linkSystem.getCount(tet.b, pk) <= 1)
    					 && (linkSystem.getCount(tet.d, pk) <= 1)) {
                Tet temp = new Tet(centerOfPlanet, tet.b, tet.d, pk);
                temp.calc();
	    		      // Now check against all nearby points
    						boolean ok = true;
    						int check = 0;
            ...
            Tet temp = new Tet(centerOfPlanet, tet.b, tet.d, bestPoint);
    				temp.calc();
						System.out.print("The best possible tet for this side involved a point at ("+bestPoint.getX()+","+bestPoint.getY()+","+bestPoint.getZ()+") ");
						System.out.println("whose closest stopper had a distanceRatio of "+bestDist+" away.");
						System.out.println("The circumradius was inner="+Math.sqrt(temp.innersq)+", outer="+Math.sqrt(temp.outersq));
          }
        } else System.out.println("t3 is NOT null");
        failedToExpand = true;
      }
  	}
    if (failedToExpand) {
    ...
    System.out.println(tets.size()+" tets");
  }
  ...
  public void saveJPG() {
  	long time = System.currentTimeMillis();
    // Render to JPG
  	int i = imageSettings.singleSize();
  	int imageWidth = i;
  	if (!imageSettings.singleSquare())
	    imageWidth = i * 2;
  	image = new BufferedImage(imageWidth, i, 1);
  	renderImage(true, image, imageWidth, i, imageSettings.singleFaults());
    
    // Now save it
    try {
	    int num;
      if(!new File("TectonicPlanet/Images").exists()) new File("TectonicPlanet/Images").mkdirs();
  ...
  public void saveJPGsequence() {
    if (imageSequenceNumber==-1) {
		  imageSequenceNumber=1;
  		while (new File("TectonicPlanet/Images/Image"+imageSequenceNumber+"_1.jpg").exists() || 
			       new File("TectonicPlanet/Images/Image"+imageSequenceNumber+"_"+numberCode(1)+".jpg").exists() || 
			       new File("TectonicPlanet/Images/Image"+numberCode(imageSequenceNumber)+"_"+numberCode(1)+".jpg").exists()) {imageSequenceNumber++;}
		}
  	long time = System.currentTimeMillis();
    // Render to JPG
  	int i = imageSettings.seqSize();
  	int imageWidth = i;
  	if (!imageSettings.seqSquare())
	    imageWidth = i * 2;
  	image = new BufferedImage(imageWidth, i, 1);
  	renderImage(false, image, imageWidth, i, imageSettings.seqFaults());
    // Now save it
    try {
	    int num;
      if(!new File("TectonicPlanet/Images").exists()) new File("TectonicPlanet/Images").mkdirs();
	    for (num = 1; new File("TectonicPlanet/Images/Image"+numberCode(imageSequenceNumber)+"_"+numberCode(num)+".jpg").exists(); num++) {
		    /* empty */
	    }
	    File file = new File("TectonicPlanet/Images/Image"+numberCode(imageSequenceNumber)+"_"+numberCode(num)+".jpg");
	    //ImageIO.write(image, "jpg", file);
      saveJPG(image, file, 1);
    } catch (Exception exception) {
      System.out.println("Exception! - "+exception);
  ...
  private void renderImage(boolean single, BufferedImage image, int width, int height, boolean showFaults) {
	  if (single) {
      if (imageSettings.singleCylindrical()) renderCylindrical(image,width,height,showFaults, imageSettings.singleAgeDots());
      if (imageSettings.singleEllipse()) renderElliptical(image,width,height,showFaults, imageSettings.singleAgeDots());
		} else {
      if (imageSettings.seqCylindrical()) renderCylindrical(image,width,height,showFaults, imageSettings.seqAgeDots());
      if (imageSettings.seqEllipse()) renderElliptical(image,width,height,showFaults, imageSettings.seqAgeDots());
		}
  }
  ...
  private void renderCylindrical(BufferedImage image, int width, int height, boolean showFaults, boolean showAgeDots) {
    ...
    for (int x=0; x<width; x++) {
      if (x%20==0) System.out.println((x*100/width)+"%");
      lon=(double)x/width*2*Math.PI;
      for (int y=0; y<height; y++) {
        lat=-Math.PI/2+(double)y/(height-1)*Math.PI;
        pos.x=Math.sin(lon)*Math.cos(lat)*planetRadius;
        pos.y=Math.sin(lat)*planetRadius;
        pos.z=Math.cos(lon)*Math.cos(lat)*planetRadius;
        ...
        if (tet != null) {
          ...
          if (dist2>dist3) {
            ...
            image.setRGB(x,height-y-1,colorMap.mapToRGB(hasl));
            if (hasl>colorMap.maxValue()) {
              //System.out.println(hasl+" -> "+tcol);
              image.setRGB(x,height-y-1,colorMap.topRGB());
            }
  ...
  private void renderElliptical(BufferedImage image, int width, int height, boolean showFaults, boolean showAgeDots) {
    ...
    for (int y1=0; y1<height; y1++) {
      ...
      lat=-Math.PI/2+(double)y/(height-1)*Math.PI;
      if (y%20==0) System.out.println((100-y*100/height)+"%");
              ...
              dist3=temp;
				  ...
				  dist2=Math.sqrt(dist2);
					if (showFaults && (closest1.getPlate()!=closest2.getPlate())
						...
						b=area2*temp;
						c=area3*temp;
						image.setRGB(x,y1,colorMap.map(closest1.getSurfaceHeight()*a+closest2.getSurfaceHeight()*b+closest3.getSurfaceHeight()*c - TecPoint.seaLevel).getRGB());
				  }
        }
			}
    }
  ...
  public void saveSatelliteData(double lat, double lon, double rotation, double imageSize, int res, String name) {
    ...
    try {
      ...
      C=lon/360 * planetRadius*2*Math.PI*1000; // Easting (in metres)
      F=lat/360 * planetRadius*2*Math.PI*1000; //Northing (in metres)
    ...
    for (int i=0; i<res; i++)
      for (int j=0; j<res; j++) {
        ...
        pos2.scale(planetRadius/(new Vector3d(pos2).length()));
        tet=getTet(pos2,previousTets);
        if (!previousTets.contains(tet)) previousTets.add(tet);
        TecPoint tp1 = tet.b;
        TecPoint tp2 = tet.c;
        TecPoint tp3 = tet.d;
  ...
  public void saveQuadsets(int num, int levels, int size) {
  	...
  	System.out.println(2*Math.PI * planetRadius	/ (Math.pow(2.0, (double) (levels - 1)) * (double) num * (double) size) + "km/pixel at the highest resolution");
        ...
        for (int col = 0;((double) col < (double) num * Math.pow(2.0, (double) lev)); col++) {
          ...
          for (int x = 0; x < size; x++) {
      			...
      			for (int y = 0; y < size; y++) {
    			    ...
    			    point3d.x = (Math.sin(lon) * Math.cos(lat) * planetRadius);
    			    point3d.y = Math.sin(lat) * planetRadius;
    			    point3d.z = (Math.cos(lon) * Math.cos(lat) * planetRadius);
    			    if (tet == null || !tet.strictlyContains(point3d)) tet = getTet(point3d, vector);
    			    if (tet != null) {
                ...
                image.setRGB(x, size - y - 1, colorMap.map(d_345_ - TecPoint.seaLevel).getRGB());
              }
            }
          }
  		    try {
  ...
  public double plateSplitScore(TecPlate tecplate) {
    ...
    if (max * max * Math.PI > ((double) (5 * checkPoints.size()) * Math.PI * (double) km * (double) km / 4.0))	    return -3.0;
    ...
    //return Math.sqrt(total1.length()*n1*n2)*rev;
  	...
  	if (crossSection == 0.0) return -2.0;
    
    // Try total1.dot(plate.splitVector)*... instead of total1.length()*rev*...
    return (vector3d.dot(tecplate.splitVector) * Math.pow(n1 * n2 * Math.pow((double) km / 200.0, 4.0), 2.0) / crossSection);
  }
    ...
    for (int i = 0; i < vector.size(); i++) {
  	...
  	for (int i = 0; i < vector.size(); i++) {
	    for (int i_454_ = 0; i_454_ < vector.size(); i_454_++) {
    		...
    		if (d_455_ > 3.141592653589793) d_455_ = 6.283185307179586 - d;
    		d = Math.max(d, d_455_);
	    }
  	}
        ...
        if (length >= d * 2.2) {
          ...
          ///////////////////////////////////////////////////////////////
  		    ...
  		    TecPoint np1 = new TecPoint((tecpoint.getPos().x * 2.0 / 3.0 + tecpoint2.getPos().x / 3.0),
                        				      ...
                        				      (tecpoint.getPos().z * 2.0 / 3.0 + tecpoint2.getPos().z / 3.0), tecpoint.getPlate(), epoch);
          TecPoint np2 = (new TecPoint((tecpoint.getPos().x / 3.0 + tecpoint2.getPos().x * 2.0 / 3.0),
                              			   ...
                              			   (tecpoint.getPos().z / 3.0 + tecpoint2.getPos().z * 2.0 / 3.0), tecpoint2.getPlate(), epoch));
  		    np1.setHeight(planetRadius);
  		    np2.setHeight(planetRadius);
  		    np1.setSize((double) (km / 2));
  		    np2.setSize((double) (km / 2));
  ...
  public void killPoint(TecPoint tecpoint) {
  	...
  	for (int i = 0; i < tets.size(); i++) {
  	  if (((Tet) tets.get(i)).uses(tecpoint)) {
    		Tet tet = (Tet) tets.get(i);
    		tets.remove(tet);
    		linkSystem.removeLink(tet.b, tet.c);
    		linkSystem.removeLink(tet.b, tet.d);
  ...
  public Tet getTet(Point3d point3d) {
	  ...
	  for (int i = 0; i < tets.size(); i++) {
	    if (((Tet) tets.get(i)).strictlyContains(point3d))
		    return (Tet) tets.get(i);
	  }
	  return null;
  }
  ...
  private void resetTetGridSystem() {
    for (int i = 0; i < tetGridSize; i++) {
	    for (int i_548_ = 0; i_548_ < tetGridSize; i_548_++) {
        for (int i_549_ = 0; i_549_ < tetGridSize; i_549_++)
          tetGridBox[i][i_548_][i_549_] = null;
	    }
    }
    for (int i = 0; i < tets.size(); i++)
	    gridBoxAdd((Tet) tets.get(i));
  }
    
  private int getTetGridBoxX(double d) {
	  return (int) Math.rint((d + planetRadius) * (double) (tetGridSize - 1) / (planetRadius * 2.0));
  }
  ...
  public void gridBoxAdd(Tet tet) {
  	...
  	try {
	    if (tetGridBox[i][i_550_][i_551_] == null)
        tetGridBox[i][i_550_][i_551_] = new HashSet(100);
	    tetGridBox[i][i_550_][i_551_].add(tet);
    } catch (Exception exception) {
  ...
  private HashSet getTetGridBox(Point3d point3d) {
  	...
  	return tetGridBox[i][i_552_][i_553_];
  }
  ...
  public HashSet tetsSurrounding(Point3d point3d, int i) {
  	...
  	for (int dx = -i; dx <= i; dx++) {
        ...
        for (int dz = -i; dz <= i; dz++) {
          if (x+dx>=0 && x+dx<tetGridSize && y+dy>=0 && y+dy<tetGridSize && z+dz>=0 && z+dz<tetGridSize && (tetGridBox[x + dx][y + dy][z + dz]) != null && (dx <= -i || dx >= i || dy <= -i || dy >= i || dz <= -i || dz >= i))
            out.addAll(tetGridBox[x + dx][y + dy][z + dz]);
        }
	    }
    }
    return out;
  ...
  public void removeTet(Tet tet) {
  	...
  	linkSystem.removeLink(tet.b, tet.c);
  	linkSystem.removeLink(tet.d, tet.c);
  	linkSystem.removeLink(tet.b, tet.d);
  }
