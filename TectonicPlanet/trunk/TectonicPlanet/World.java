...
public class World {
    ...
    public void delaunay(){
        ...
        if( centerOfPlanet == null )
            centerOfPlanet = new TecPoint( 0.0, 0.0, 0.0, epoch );
    ...
    public int getPlateNum( TecPlate tp ){
        for( int i = 0; i < plates.size(); i++ )
            if( tp.equals( plates.get( i ) ) )
                return i;
        return -1;
    }

    public TecPlate getPlate( int i ){
        return (TecPlate)plates.get( i );
    }
    ...
    private int getGridBoxSize( int i, int i_117_, int i_118_ ){
        if( gridBox[i][i_117_][i_118_] == null )
            return 0;
...
//System.out.println("Added new tet");
            ...
            if( linkSystem.getCount( tet.b, tet.c ) >= 2
                ...
                if( points.contains( tet.a ) )
                    System.out.println( " and is point #" + points.indexOf( tet.a ) );
                else
                    System.out.println( " and isn't in points!" );
                System.out.print( "tet.b=(" + tet.b.getX() + "," + tet.b.getY() + "," + tet.b.getZ() + ")" );
                if( points.contains( tet.b ) )
                    System.out.println( " and is point #" + points.indexOf( tet.b ) );
                else
                    System.out.println( " and isn't in points!" );
                System.out.print( "tet.c=(" + tet.c.getX() + "," + tet.c.getY() + "," + tet.c.getZ() + ")" );
                if( points.contains( tet.c ) )
                    System.out.println( " and is point #" + points.indexOf( tet.c ) );
                else
                    System.out.println( " and isn't in points!" );
                System.out.print( "tet.d=(" + tet.d.getX() + "," + tet.d.getY() + "," + tet.d.getZ() + ")" );
                if( points.contains( tet.d ) )
                    System.out.println( " and is point #" + points.indexOf( tet.d ) );
                else
                    System.out.println( " and isn't in points!" );
                if( t1 == null ){
    ...
    public void saveJPGsequence(){
        if( imageSequenceNumber == -1 ){
            ...
            while( new File( "TectonicPlanet/Images/Image" + imageSequenceNumber + "_1.jpg" ).exists()
                    ...
                    || new File( "TectonicPlanet/Images/Image" + numberCode( imageSequenceNumber ) + "_" + numberCode( 1 ) + ".jpg" ).exists() ){
                imageSequenceNumber++;
            }
        }
        long time = System.currentTimeMillis();
    ...
    private void renderImage( boolean single, BufferedImage image, int width, int height, boolean showFaults ){
        if( single ){
            if( imageSettings.singleCylindrical() )
                renderCylindrical( image, width, height, showFaults, imageSettings.singleAgeDots() );
            if( imageSettings.singleEllipse() )
                renderElliptical( image, width, height, showFaults, imageSettings.singleAgeDots() );
        }
        else {
            if( imageSettings.seqCylindrical() )
                renderCylindrical( image, width, height, showFaults, imageSettings.seqAgeDots() );
            if( imageSettings.seqEllipse() )
                renderElliptical( image, width, height, showFaults, imageSettings.seqAgeDots() );
        }
    }
    ...
    public double plateSplitScore( TecPlate tecplate ){
        ...
        if( max * max * Math.PI > ((double)(5 * checkPoints.size()) * Math.PI * (double)km * (double)km / 4.0) )
            return -3.0;
